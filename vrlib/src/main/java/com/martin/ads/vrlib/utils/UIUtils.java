...
//                        - TimeUnit.HOURS.toMinutes(TimeUnit.MILLISECONDS.toHours(milliseconds)),
    ...
    //缓冲动画控制
    public static void setBufferVisibility(ImageView imgBuffer, ImageView imgBufferTop, boolean Visible) {
        if (Visible) {
            imgBuffer.setVisibility(View.VISIBLE);
            imgBufferTop.setVisibility(View.VISIBLE);
            imgBufferTop.setBackgroundResource(R.drawable.gs_bg);
            startImageAnim(imgBuffer, R.drawable.loading);
            ToastUtils.showShort("直播加载中，请稍等...");
        } else {
            stopImageAnim(imgBuffer);
            imgBuffer.setVisibility(View.GONE);
            imgBufferTop.setVisibility(View.GONE);
        }
    }
